...
######### Mamba mixer
framewise_mixer = FramewiseMixer(embed_dim=64, num_heads=4, ff_dim=128, mode='sequential_transformer_first')
...
print(f" RAI Backbone output {conv23_rai.shape} and RDI Backbone output {conv23_rdi.shape}")

rai_mixer_out=framewise_mixer(conv23_rai)
rdi_mixer_out=framewise_mixer(conv23_rdi)
conv23 = tf.keras.layers.Concatenate(axis=-1)([rai_mixer_out,rdi_mixer_out])

print(f" size after concatenation {conv23.shape}")

gap_op = tf.keras.layers.GlobalAveragePooling3D()(conv23)
